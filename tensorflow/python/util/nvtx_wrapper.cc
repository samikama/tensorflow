  ...
  m.def("push",
        [](const std::string& message,
          ...
          if (tensorflow::nvtx_helper::is_nvtx_on()) {
            token->range = tensorflow::nvtx_helper::StartNvtxRange(
                message.c_str(), type.c_str());
          }
          return token;
        });
  m.def("pop", [](tensorflow::nvtx_utils::PyNVTXToken* token) {
    if (tensorflow::nvtx_helper::is_nvtx_on() && token) {
      tensorflow::nvtx_helper::EndNvtxRange(token->range);
    }
  });
}
