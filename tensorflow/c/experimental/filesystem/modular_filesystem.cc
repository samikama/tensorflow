...
Status ModularFileSystem::StartTransaction(TransactionToken** token) {
  if (ops_->start_transaction == nullptr) {
    if (VLOG_IS_ON(4)) {
      VLOG(4) << "This filesystem plugin do not implement StartTransaction";
      token = nullptr;
      return Status::OK();
    }
  }
  TF_TransactionToken* tok;
  UniquePtrTo_TF_Status plugin_status(TF_NewStatus(), TF_DeleteStatus);
  ops_->start_transaction(filesystem_.get(), &tok, plugin_status.get());
  if (!tok) {
    *token = nullptr;
  } else {
    *token = new TransactionToken();
    ...
    transaction_map_.insert({tok, *token});
  }
  return StatusFromTF_Status(plugin_status.get());
}

Status ModularFileSystem::EndTransaction(TransactionToken* token) {
  if (!token || (ops_->end_transaction == nullptr)) {
    return Status::OK();
  }
  if (token->owner != this) {
    return errors::InvalidArgument(
        "Invalid token. This token does not belong to this FileSystem");
  }
  TF_TransactionToken* tok = static_cast<TF_TransactionToken*>(token->token);
  UniquePtrTo_TF_Status plugin_status(TF_NewStatus(), TF_DeleteStatus);
  ops_->end_transaction(filesystem_.get(), tok, plugin_status.get());
  {
    mutex_lock l(transaction_mutex_);
    transaction_map_.erase(static_cast<TF_TransactionToken*>(token->token));
  }
  delete token;
  return StatusFromTF_Status(plugin_status.get());
}

Status ModularFileSystem::AddToTransaction(const string& path,
                                           TransactionToken* token) {
  if (!token || (ops_->add_to_transaction == nullptr)) return Status::OK();
  if (token->owner != this) {
    return errors::InvalidArgument(
        "Invalid token. This token does not belong to this FileSystem");
  }
  TF_TransactionToken* tok = static_cast<TF_TransactionToken*>(token->token);
  UniquePtrTo_TF_Status plugin_status(TF_NewStatus(), TF_DeleteStatus);
  std::string translated_path = TranslateName(path);
  ops_->add_to_transaction(filesystem_.get(), translated_path.c_str(), tok,
                           plugin_status.get());
  return StatusFromTF_Status(plugin_status.get());
}

Status ModularFileSystem::GetTransactionForPath(const string& path,
                                                TransactionToken** token) {
  if (ops_->get_transaction_for_path == nullptr) {
    *token = nullptr;
    return Status::OK();
  }
  TF_TransactionToken* tok = nullptr;
  UniquePtrTo_TF_Status plugin_status(TF_NewStatus(), TF_DeleteStatus);
  std::string translated_path = TranslateName(path);
  ops_->get_transaction_for_path(filesystem_.get(), translated_path.c_str(),
                                 &tok, plugin_status.get());
  *token = nullptr;
  if (tok) {
    {
      mutex_lock l(transaction_mutex_);
      auto res = transaction_map_.insert({tok, nullptr});
      if (res.second) {
        *token = new TransactionToken();
        (*token)->owner = this;
        (*token)->token = tok;
        res.first->second = *token;
      } else {
        *token = res.first->second;
      }
    }
  }
  return StatusFromTF_Status(plugin_status.get());
...
Status ModularFileSystem::GetTokenOrStartTransaction(const string& path,
                                                     TransactionToken** token) {
  if (ops_->get_or_start_transaction_for_path == nullptr) {
    *token = nullptr;
    return Status::OK();
  }
  TF_TransactionToken* tok = nullptr;
  UniquePtrTo_TF_Status plugin_status(TF_NewStatus(), TF_DeleteStatus);
  std::string translated_path = TranslateName(path);
  ops_->get_or_start_transaction_for_path(
      filesystem_.get(), translated_path.c_str(), &tok, plugin_status.get());
  *token = nullptr;
  if (tok) {
    {
      mutex_lock l(transaction_mutex_);
...
string ModularFileSystem::DecodeTransaction(const TransactionToken* token) {
  ...
  if (token->owner != this) {
    return "This token do not belong to this filesystem!";
  }
  TF_TransactionToken* tok = static_cast<TF_TransactionToken*>(token->token);
  if (!ops_->decode_transaction_token) {
    if (!tok) {
      return "Invalid token!";
    }
    std::stringstream oss;
    oss << "Token= " << (void*)tok->token << ", Owner= " << (void*)tok->owner;
    return oss.str();
  }

  char* buff = ops_->decode_transaction_token(filesystem_.get(), tok);
  string result(buff);
  plugin_memory_free_(buff);
  return result;
}
