...
typedef struct TF_FilesystemOps {
  ...
  ///
  /// Opaque transaction token is returned in `token`. Ownership of the token is
  ...
  /// Adds file/directory in the `path` to transaction in `token`. It is a valid operation to add a path that doesn't exist yet to a transaction.
  ///
  /// In case of error, plugins must set `status` to a value different than
  /// `TF_OK`, free memory allocated for `token` and return -1.
  ///
  ...
  /// Returns transaction token for `path` if it is part of a transaction else starts a new transaction and adds `path` to that transaction
  ///
  /// Transaction token is returned in `token`. Ownership of the token is in filesystem. 
  /// Token will be freed in `end_transaction` call and any access to token after that is invalid.
  ///
  /// In case of error, plugins must set `status` to a value different than
  /// `TF_OK`, free memory allocated for `token` and return -1.
  ///
  ...
  ///
  /// Plugins:
  ///   * Must set `status` to `TF_OK` if transaction found or successfuly started.
  ...
  ///   * Might use any other error value for `status` to signal other errors.
  int (*get_or_start_transaction_for_path)(const TF_Filesystem* filesystem, const char* path,
                      TF_TransactionToken **token, TF_Status* status);

  /// Decodes transaction token in `token` to human readable format for debugging.
  ///
  /// A new `char*` buffer must be allocated by this method. Core TensorFlow
  /// manages the lifetime of the buffer after the call. Thus, all callers of
  /// this method must take ownership of the returned pointer.
  ...
  char* (*decode_transaction_token)(const TF_Filesystem* filesystem, const TF_TransactionToken* token);
