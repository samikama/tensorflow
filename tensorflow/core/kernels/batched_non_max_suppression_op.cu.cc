...
__global__ void BroadcastBoxes(const int num_boxes, const int num_classes,
                               ...
                               Box* output_boxes) {
  constexpr int cache_size = 3000;
  __shared__ Box BoxCache[cache_size];
  int per_batch_cache = cache_size / blockDim.z;
  int num_chunks = (num_boxes + per_batch_cache - 1) / per_batch_cache;
  for (int b : GpuGridRangeZ(batch_size)) {
    int batch_offset =
        b * num_boxes;  // input boxes are given only once per batch
    for (int c = 0; c < num_chunks; ++c) {
      int chunk_start = c * cache_size;
      int chunk_end = min(num_boxes - chunk_start, per_batch_cache);
      int cache_offset = (threadIdx.z % blockDim.z) * per_batch_cache;
      if (threadIdx.y == 0) {
        for (int x : GpuGridRangeX(chunk_end)) {
          BoxCache[x + cache_offset] =
              input_boxes[batch_offset + chunk_start + x];
        }
      }
      __syncthreads();
      for (int cl : GpuGridRangeY(num_classes)) {
        int cl_offset =
            cl * num_boxes + batch_offset * num_classes + chunk_start;

        for (int x : GpuGridRangeX(chunk_end)) {
          output_boxes[cl_offset + x] = BoxCache[x + cache_offset];
        }
      }
      __syncthreads();
    }
  ...
  for (int b : CudaGridRangeZ(num_batches)) {
    ...
    for (int c : CudaGridRangeY(num_classes)) {
      ...
      for (int idx : CudaGridRangeX(num_selections[b * num_classes + c])) {
        ...
        Box* bb = batch_out_boxes + out_offset + idx;
      }
    }
...
Status DoNMSBatched(OpKernelContext* context, const Tensor& boxes,
  ...
  if (VLOG_IS_ON(5)) {
    string botmp = "box offsets=[ ";
    string above_score = "Above score threshold= [ ";
    std::vector<int> counts_for_selection(batch_size, 0);
    device.memcpyDeviceToHost(counts_for_selection.data(), device_box_counts,
                              sizeof(int) * batch_size);
    device.synchronize();
    for (int i = 0; i < batch_size; ++i) {
      StrAppend(&botmp, begin_end_offsets[i], " ");
    }
    for (const auto o : counts_for_selection) {
      StrAppend(&above_score, o, " ");
    }
    VLOG(1) << botmp << "] " << above_score << "]";
  }
  ...
  if (VLOG_IS_ON(5)) {
    int num_selected = 0;
    device.memcpyDeviceToHost(&num_selected, device_selected_counts,
                              sizeof(int));
    device.synchronize();
    auto selected_str =
        StrCat("selected for kernel ", kernel, " after cub flagged= [ ");
    StrAppend(&selected_str, num_selected, " ], counts from kernels = [");
    int sum = 0;

    for (int i = 0; i < batch_size; ++i) {
      StrAppend(&selected_str, selected_counts[i], " ");
      sum += selected_counts[i];
    }
    VLOG(1) << selected_str << "] total=" << sum;
  }
  ...
  if (VLOG_IS_ON(5)) {
    Tensor h_selected_indices;
    TF_RETURN_IF_ERROR(context->allocate_temp(
        DataType::DT_INT32, TensorShape({max_boxes * batch_size * 2}),
        &h_selected_indices, alloc_attr));
    device.memcpyDeviceToHost(h_selected_indices.flat<int>().data(),
                              d_selected_indices.flat<int>().data(),
                              d_selected_indices.NumElements() * 4);
    device.synchronize();
    string msg("Device Selected\n");
    string msg2("Post_scatter\n");
    int* hsel = h_selected_indices.flat<int>().data();
    int* hscat = hsel + batch_size * max_boxes;
    int bseloff = 0;
    for (int ba = 0; ba < batch_size; ba++) {
      StrAppend(&msg, "Batch ", ba, ": ");
      StrAppend(&msg2, "Batch ", ba, ": ");
      for (int s = 0; s < selected_counts[ba]; s++) {
        StrAppend(&msg, hsel[bseloff + s], ", ");
        StrAppend(&msg2, hscat[ba * max_boxes + s], ", ");
      }
      bseloff += selected_counts[ba];
      StrAppend(&msg, "\n");
      StrAppend(&msg2, "\n");
    }
    VLOG(1) << msg << msg2;
  ...
  if (VLOG_IS_ON(5)) {
    device.synchronize();
    TF_RETURN_IF_CUDA_ERROR(cudaGetLastError());
    VLOG(1) << "DoBatched done ] output= "
            << (*output_indices)->DeviceSafeDebugString();
  }
  return Status::OK();
}
...
class BatchedNonMaxSuppressionGPUOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    auto device = context->eigen_gpu_device();
    // device.memset(num_outputs_t->flat<int>().data(), 0,
    //               sizeof(int) * batch_size);
...
class CombinedNonMaxSuppressionGPUOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (boxes.NumElements() == 0) {  // Empty input
      ...
      OP_REQUIRES_OK(
          ...
          context->allocate_output(
              0, TensorShape({batch_size, max_total_size_per_batch, 4}),
              &output_boxes_tensor));
      Tensor* output_scores_tensor = nullptr;
      OP_REQUIRES_OK(context,
                     context->allocate_output(
                         1, TensorShape({batch_size, max_total_size_per_batch}),
                         &output_scores_tensor));
      Tensor* output_classes_tensor = nullptr;
      OP_REQUIRES_OK(context,
                     context->allocate_output(
                         2, TensorShape({batch_size, max_total_size_per_batch}),
                         &output_classes_tensor));
      Tensor* final_counts_tensor = nullptr;
      OP_REQUIRES_OK(context,
                     context->allocate_output(3, TensorShape({batch_size}),
                                              &final_counts_tensor));
      return;
    }
    AllocatorAttributes alloc_attr;
    ...
    Tensor reshaped_boxes_tensor, transposed_boxes_tensor;
    OP_REQUIRES_OK(context,
                   context->allocate_temp(
                       DataType::DT_FLOAT,
                       TensorShape({batch_size, num_classes, num_boxes, 4}),
                       &transposed_boxes_tensor));
    ...
    if ((boxes.dim_size(2) == 1) &&
        scores.dim_size(2) != 1) {  // need broadcast
      const Box* in_boxes =
          reinterpret_cast<const Box*>(boxes.flat<float>().data());
      Box* out_boxes =
          reinterpret_cast<Box*>(transposed_boxes_tensor.flat<float>().data());
      Gpu3DLaunchConfig config2d = GetGpu3DLaunchConfig(
          num_boxes, num_classes, batch_size, device, BroadcastBoxes, 0, 1024);

      OP_REQUIRES_OK(context,
                     GpuLaunchKernel(BroadcastBoxes, config2d.block_count,
                                     config2d.thread_per_block, 0,
                                     device.stream(), num_boxes, num_classes,
                                     batch_size, in_boxes, out_boxes));
      OP_REQUIRES(
          context,
          reshaped_boxes_tensor.CopyFrom(
              transposed_boxes_tensor,
              TensorShape({batch_size * num_classes, num_boxes, 4})),
          errors::Internal("Reinterpreting boxes as [batch*num_classes, "
                           "num_boxes, 4] tensor failed"));
      OP_REQUIRES_OK(context,
                     tensorflow::DoTranspose(device, scores, {{0, 2, 1}},
                                             &transposed_scores_tensor));
      OP_REQUIRES(
          context,
          reshaped_scores_tensor.CopyFrom(
              transposed_scores_tensor,
              TensorShape({batch_size * num_classes, num_boxes})),
          errors::Internal("Reinterpreting scores as [batch*num_classes, "
                           "num_boxes] tensor failed"));

    } else if (scores.dim_size(2) != 1) {  // need transpose
      OP_REQUIRES_OK(context,
                     tensorflow::DoTranspose(device, boxes, {{0, 2, 1, 3}},
                                             &transposed_boxes_tensor));
      OP_REQUIRES_OK(context,
                     tensorflow::DoTranspose(device, scores, {{0, 2, 1}},
                                             &transposed_scores_tensor));
      ...
      OP_REQUIRES(
          context,
          reshaped_boxes_tensor.CopyFrom(
              boxes, TensorShape({batch_size * num_classes, num_boxes, 4})),
          errors::Internal("Reinterpreting boxes as [batch*num_classes, "
                           "num_boxes, 4] tensor failed"));
      OP_REQUIRES(
          context,
          reshaped_scores_tensor.CopyFrom(
              scores, TensorShape({batch_size * num_classes, num_boxes})),
          errors::Internal("Reinterpreting scores as [batch*num_classes, "
                           "num_boxes] tensor failed"));
    }
    ...
    OP_REQUIRES_OK(
        context,
        DoNMSBatched(context, reshaped_boxes_tensor, reshaped_scores_tensor,
                     box_counts_tensor, max_size_per_class, iou_threshold_val,
                     score_threshold_val, true, per_class_outputs,
                     &output_indices_ptr, 0));
    if (VLOG_IS_ON(1)) {
      Tensor tmp_indices;
      OP_REQUIRES_OK(context,
                     context->allocate_temp(DataType::DT_INT32,
                                            output_indices_tensor.shape(),
                                            &tmp_indices, alloc_attr));
      device.memcpyDeviceToHost(
          tmp_indices.flat<int>().data(),
          output_indices_tensor.flat<int>().data(),
          output_indices_tensor.NumElements() * sizeof(int));
      device.synchronize();
      VLOG(1) << "\nIndices=" << tmp_indices.DebugString(-1);
    }
    ...
    Tensor gathered_scores_tensor, sorted_scores_tensor;
    OP_REQUIRES_OK(
        context,
        context->allocate_temp(
            DataType::DT_FLOAT,
    ...
    Tensor out_sort_indices_tensor, sorted_indices_tensor;
    OP_REQUIRES_OK(
        context,
        context->allocate_temp(
            DataType::DT_INT32,
    ...
    int* indices = output_indices_tensor.flat<int>().data();
    Box* reshaped_boxes =
        reinterpret_cast<Box*>(reshaped_boxes_tensor.flat<float>().data());
    float* reshaped_scores = reshaped_scores_tensor.flat<float>().data();
    Box* out_boxes =
    ...
    if (clip_boxes_) {
      ...
      OP_REQUIRES_OK(
          ...
          GpuLaunchKernel(
              ...
              config3d.thread_per_block, 0, device.stream(), indices,
              max_size_per_class, reshaped_boxes, reshaped_scores,
              per_class_outputs, num_boxes, batch_size, num_classes,
    ...
    cudaError_t cuda_ret = cub::DeviceSegmentedRadixSort::SortPairsDescending(
        ...
        static_cast<int*>(nullptr),    // input indices
        static_cast<int*>(nullptr),    // sorted indices
        batch_size * max_size_per_class *
            num_classes,  // Total number of boxes in batch
        batch_size,       // num segments
        static_cast<int*>(nullptr), static_cast<int*>(nullptr), 0,
        8 * sizeof(float),  // sort all bits
        cuda_stream);
    ...
    Tensor cub_temp_buffer_tensor;
    OP_REQUIRES_OK(context, context->allocate_temp(
                                DataType::DT_INT8,
                                TensorShape({(int64)cub_temp_storage_bytes}),
                                &cub_temp_buffer_tensor));
    float* sorted_scores = sorted_scores_tensor.flat<float>().data();
    int* sorted_indices = sorted_indices_tensor.flat<int>().data();
    cuda_ret = cub::DeviceSegmentedRadixSort::SortPairsDescending(
        cub_temp_buffer_tensor.flat<int8>().data(), cub_temp_storage_bytes,
        out_scores,        // scores
        sorted_scores,     // sorted scores
        out_sort_indices,  // input indices
        sorted_indices,    // sorted indices
        batch_size * max_size_per_class *
            num_classes,  // Total number of boxes in batch
        batch_size,       // num segments
        merged_counts, merged_counts + batch_size, 0,
        8 * sizeof(float),  // sort all bits
        cuda_stream);
    TF_OP_REQUIRES_CUDA_SUCCESS(context, cuda_ret);
    TF_OP_REQUIRES_CUDA_SUCCESS(context, cudaGetLastError());
    ...
    OP_REQUIRES_OK(
        ...
        GpuLaunchKernel(
            ...
            sorted_indices, out_boxes, final_boxes, out_scores, final_scores,
