...
Status NmsGpu(const float* d_sorted_boxes_float_ptr, const int num_boxes,
  ...
  const int bit_mask_len =
      (num_boxes + NMSGPU::kNmsBoxesPerThread - 1) / NMSGPU::kNmsBoxesPerThread;
  ...
  int num_blocks = (num_boxes + NMSGPU::kNmsBlockDim - 1) / NMSGPU::kNmsBlockDim;
  num_blocks = std::max(std::min(num_blocks, NMSGPU::kNmsBlockDimMax), 1);
  block_dim.x = num_blocks;
  block_dim.y = num_blocks;
  block_dim.z = 1;
  thread_block.x = NMSGPU::kNmsBlockDim;
  thread_block.y = NMSGPU::kNmsBlockDim;
  thread_block.z = 1;
  if (flip_boxes) {
    TF_CHECK_OK(GpuLaunchKernel(
        NMSGPU::NMSKernel<true>, block_dim, thread_block, 0, device.stream(),
        d_sorted_boxes, num_boxes, iou_threshold, bit_mask_len, d_delete_mask));
  } else {
    TF_CHECK_OK(GpuLaunchKernel(
        NMSGPU::NMSKernel<false>, block_dim, thread_block, 0, device.stream(),
        d_sorted_boxes, num_boxes, iou_threshold, bit_mask_len, d_delete_mask));
  }
  ...
  TF_CHECK_OK(GpuLaunchKernel(NMSGPU::Iota<int>, config.block_count,
                              config.thread_per_block, 0, device.stream(),
  ...
  TF_CHECK_OK(GpuLaunchKernel(
      NMSGPU::NMSReduce, 1, 1024, bit_mask_len * sizeof(int), device.stream(),
      d_delete_mask, bit_mask_len, num_boxes, max_boxes, selected));
  TF_RETURN_IF_CUDA_ERROR(cudaGetLastError());
...
Status DoNMS(OpKernelContext* context, const Tensor& boxes,
  ...
  TF_CHECK_OK(GpuLaunchKernel(NMSGPU::IndexMultiSelect<int, float4>,
  ...
  if (score_threshold > std::numeric_limits<float>::lowest()) {
    NMSGPU::GreaterThanCubOp score_limit(score_threshold);
    TF_RETURN_IF_ERROR(NMSGPU::CountIf(context, d_sorted_scores.flat<float>().data(),
  ...
  TF_CHECK_OK(GpuLaunchKernel(
      NMSGPU::IndexMultiSelect<int, int>, config.block_count,
      config.thread_per_block, 0, device.stream(), config.virtual_thread_count,
