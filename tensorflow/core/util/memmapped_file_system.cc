...
Status MemmappedFileSystem::FileExists(const string& fname, std::unique_ptr<TransactionToken>* token) {
  if (!mapped_memory_) {
...
Status MemmappedFileSystem::NewRandomAccessFile(
    const string& filename, std::unique_ptr<RandomAccessFile>* result, std::unique_ptr<TransactionToken>* token) {
  if (!mapped_memory_) {
    return errors::FailedPrecondition("MemmappedEnv is not initialized");
  }
...
Status MemmappedFileSystem::NewReadOnlyMemoryRegionFromFile(
    const string& filename, std::unique_ptr<ReadOnlyMemoryRegion>* result, std::unique_ptr<TransactionToken>* token) {
  if (!mapped_memory_) {
    return errors::FailedPrecondition("MemmappedEnv is not initialized");
  }
  const auto dir_element = directory_.find(filename);
...
Status MemmappedFileSystem::GetFileSize(const string& filename, uint64* size, std::unique_ptr<TransactionToken>* token) {
  if (!mapped_memory_) {
    return errors::FailedPrecondition("MemmappedEnv is not initialized");
  }
  const auto dir_element = directory_.find(filename);
...
Status MemmappedFileSystem::Stat(const string& fname, FileStatistics* stat, std::unique_ptr<TransactionToken>* token) {
  uint64 size;
...
Status MemmappedFileSystem::NewWritableFile(const string& filename,
                                            std::unique_ptr<WritableFile>* wf, std::unique_ptr<TransactionToken>* token) {
  return errors::Unimplemented("memmapped format doesn't support writing");
}

Status MemmappedFileSystem::NewAppendableFile(
    const string& filename, std::unique_ptr<WritableFile>* result, std::unique_ptr<TransactionToken>* token) {
  return errors::Unimplemented("memmapped format doesn't support writing");
}

Status MemmappedFileSystem::GetChildren(const string& filename,
                                        std::vector<string>* strings, std::unique_ptr<TransactionToken>* token) {
...
Status MemmappedFileSystem::DeleteFile(const string& filename, std::unique_ptr<TransactionToken>* token) {
  return errors::Unimplemented("memmapped format doesn't support DeleteFile");
}

Status MemmappedFileSystem::CreateDir(const string& dirname, std::unique_ptr<TransactionToken>* token) {
  return errors::Unimplemented("memmapped format doesn't support CreateDir");
}

Status MemmappedFileSystem::DeleteDir(const string& dirname, std::unique_ptr<TransactionToken>* token) {
...
Status MemmappedFileSystem::RenameFile(const string& filename_from,
                                       const string& filename_to, std::unique_ptr<TransactionToken>* token) {
