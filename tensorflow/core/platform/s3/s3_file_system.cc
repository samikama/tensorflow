...
Status S3FileSystem::NewRandomAccessFile(
  ...
  return NewRandomAccessFile(fname, result, true, std::unique_ptr<TransactionToken>* token);
}

Status S3FileSystem::NewRandomAccessFile(
    const string& fname, std::unique_ptr<RandomAccessFile>* result,
    bool use_multi_part_download, std::unique_ptr<TransactionToken>* token) {
...
Status S3FileSystem::FileExists(const string& fname, std::unique_ptr<TransactionToken>* token) {
...
Status S3FileSystem::Stat(const string& fname, FileStatistics* stats, std::unique_ptr<TransactionToken>* token) {
...
Status S3FileSystem::DeleteFile(const string& fname, std::unique_ptr<TransactionToken>* token) {
...
Status S3FileSystem::CreateDir(const string& dirname, std::unique_ptr<TransactionToken>* token) {
...
Status S3FileSystem::DeleteDir(const string& dirname, std::unique_ptr<TransactionToken>* token) {
...
Status S3FileSystem::GetFileSize(const string& fname, uint64* file_size, std::unique_ptr<TransactionToken>* token) {
  FileStatistics stats;
  TF_RETURN_IF_ERROR(this->Stat(fname, &stats));
...
Status S3FileSystem::RenameFile(const string& src, const string& target, std::unique_ptr<TransactionToken>* token) {
