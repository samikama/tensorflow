...
class FileSystem {
 public:
  ...
  virtual tensorflow::Status StartTransaction(
      const string& fname, TransactionToken** token = nullptr) {return Status::OK();};

  /// \brief Starts a new transaction
  virtual tensorflow::Status EndTransaction(
      TransactionToken* token = nullptr){return Status::OK();};

  /// Returns OK if the named path exists and NOT_FOUND otherwise.
  virtual tensorflow::Status FileExists(const string& fname, TransactionToken* token=nullptr) = 0;
  ...
  virtual bool Match(const std::string& filename, const std::string& pattern, TransactionToken* token=nullptr);
...
class WrappedFileSystem : public FileSystem {
 public:
  ...
  virtual tensorflow::Status NewRandomAccessFile(
      ...
      TransactionToken* token = nullptr) {return fs_->NewRandomAccessFile(fname,result,(token?token:token_));}
  ...
  virtual tensorflow::Status NewWritableFile(
      ...
      TransactionToken* token = nullptr) {return fs_->NewWritableFile(fname,result,(token?token:token_));}
  ...
  virtual tensorflow::Status NewAppendableFile(
      const string& fname, std::unique_ptr<WritableFile>* result,
      TransactionToken* token = nullptr) {return fs_->NewAppendableFile(fname,result, (token?token:token_));}
  ...
  virtual tensorflow::Status NewReadOnlyMemoryRegionFromFile(
      ...
      TransactionToken* token = nullptr) {return fs_->NewReadOnlyMemoryRegionFromFile(fname,result, (token?token:token_));}

  /// \brief Starts a new transaction
  virtual tensorflow::Status StartTransaction(
      const string& fname, TransactionToken** token = nullptr){ 
        return fs_->StartTransaction(fname,token);
  };

  /// \brief Starts a new transaction
  virtual tensorflow::Status EndTransaction(TransactionToken* token = nullptr){
  ...
  virtual tensorflow::Status GetMatchingPaths(
      ...
      TransactionToken* token = nullptr) {return fs_->GetMatchingPaths(pattern,results, (token?token:token_));}
  ...
  virtual bool Match(const std::string& filename, const std::string& pattern,
                     TransactionToken* token = nullptr) {return fs_->Match(filename,pattern, (token?token:token_));}

  /// \brief Obtains statistics for the given path.
  virtual tensorflow::Status Stat(const string& fname, FileStatistics* stat,
                                  TransactionToken* token = nullptr) {return fs_->Stat(fname,stat, (token?token:token_));}

  /// \brief Deletes the named file.
  virtual tensorflow::Status DeleteFile(const string& fname,
                                        TransactionToken* token = nullptr) {return fs_->DeleteFile(fname, (token?token:token_));}
  ...
  virtual tensorflow::Status CreateDir(const string& dirname,
                                       TransactionToken* token = nullptr) {return fs_->CreateDir(dirname, (token?token:token_));}
  ...
  virtual tensorflow::Status RecursivelyCreateDir(
      const string& dirname, TransactionToken* token = nullptr) {return fs_->RecursivelyCreateDir(dirname, (token?token:token_));}

  /// \brief Deletes the specified directory.
  virtual tensorflow::Status DeleteDir(const string& dirname,
                                       TransactionToken* token = nullptr) {return fs_->DeleteDir(dirname, (token?token:token_));}
  ...
  virtual tensorflow::Status DeleteRecursively(
      ...
      TransactionToken* token = nullptr) {return fs_->DeleteRecursively(dirname,undeleted_files,undeleted_dirs, (token?token:token_));}

  /// \brief Stores the size of `fname` in `*file_size`.
  virtual tensorflow::Status GetFileSize(const string& fname, uint64* file_size,
                                         TransactionToken* token = nullptr) {return fs_->GetFileSize(fname,file_size, (token?token:token_));}

  /// \brief Overwrites the target if it exists.
  virtual tensorflow::Status RenameFile(const string& src, const string& target,
                                        TransactionToken* token = nullptr) {return fs_->RenameFile(src,target, (token?token:token_));}

  /// \brief Copy the src to target.
  virtual tensorflow::Status CopyFile(const string& src, const string& target,
                                      TransactionToken* token = nullptr) {return fs_->CopyFile(src,target, (token?token:token_));}
  ...
  /// paths with respect to the actual working directory.  That is, this is
  /// purely string manipulation, completely independent of process state.
  virtual std::string TranslateName(const std::string& name) const {return fs_->TranslateName(name);}
  ...
  virtual tensorflow::Status IsDirectory(const string& fname,
                                         TransactionToken* token = nullptr) {return fs_->IsDirectory(fname, (token?token:token_));}
  ...
  ///  * UNIMPLEMENTED - The file system of the path hasn't been implemented in
  ///  TF
  virtual Status HasAtomicMove(const string& path, bool* has_atomic_move) {return fs_->HasAtomicMove(path,has_atomic_move);}

  /// \brief Flushes any cached filesystem objects from memory.
  virtual void FlushCaches(TransactionToken* token = nullptr) {return fs_->FlushCaches((token?token:token_));}
  ...
  /// a user may need access to filesystems with '/' separators, such as cloud
  /// filesystems.
  virtual char Separator() const {return fs_->Separator();}

  /// \brief Split a path to its basename and dirname.
  ///
  /// Helper function for Basename and Dirname.
  std::pair<StringPiece, StringPiece> SplitPath(StringPiece uri) const {return fs_->SplitPath(uri);}

  /// \brief returns the final file name in the given path.
  ///
  /// Returns the part of the path after the final "/".  If there is no
  /// "/" in the path, the result is the same as the input.
  virtual StringPiece Basename(StringPiece path) const {return fs_->Basename(path);}
  ...
  /// leading "/".  If there is no "/" in the path, the result is the empty
  /// prefix of the input.
  StringPiece Dirname(StringPiece path) const {return fs_->Dirname(path);}

  /// \brief Returns the part of the basename of path after the final ".".
  ///
  /// If there is no "." in the basename, the result is empty.
  StringPiece Extension(StringPiece path) const {return fs_->Extension(path);}
  ...
  /// paths with respect to the actual working directory.  That is, this is
  /// purely string manipulation, completely independent of process state.
  std::string CleanPath(StringPiece path) const {return fs_->CleanPath(path);}

  /// \brief Creates a URI from a scheme, host, and path.
  ///
  /// If the scheme is empty, we just return the path.
  std::string CreateURI(StringPiece scheme, StringPiece host,
                        StringPiece path) const {return fs_->CreateURI(scheme,host,path);}

  ///  \brief Creates a temporary file name with an extension.
  std::string GetTempFilename(const std::string& extension) const {return fs_->GetTempFilename(extension);}

  /// \brief Return true if path is absolute.
  bool IsAbsolutePath(tensorflow::StringPiece path) const {return fs_->IsAbsolutePath(path);}
  ...
  std::string JoinPathImpl(
      std::initializer_list<tensorflow::StringPiece> paths) {return fs_->JoinPathImpl(paths);}
  ...
  /// empty.
  void ParseURI(StringPiece remaining, StringPiece* scheme, StringPiece* host,
                StringPiece* path) const {return fs_->ParseURI(remaining,scheme,host,path);}
