...
Status GcsFileSystem::FileExists(const string& fname, std::unique_ptr<TransactionToken>* token) {
  string bucket, object;
...
Status GcsFileSystem::Stat(const string& fname, FileStatistics* stat, std::unique_ptr<TransactionToken>* token) {
  if (!stat) {
...
Status GcsFileSystem::DeleteFile(const string& fname, std::unique_ptr<TransactionToken>* token) {
  string bucket, object;
  TF_RETURN_IF_ERROR(ParseGcsPath(fname, false, &bucket, &object));
...
Status GcsFileSystem::CreateDir(const string& dirname, std::unique_ptr<TransactionToken>* token) {
  string bucket, object;
...
Status GcsFileSystem::DeleteDir(const string& dirname, std::unique_ptr<TransactionToken>* token) {
  std::vector<string> children;
...
Status GcsFileSystem::GetFileSize(const string& fname, uint64* file_size, std::unique_ptr<TransactionToken>* token) {
  if (!file_size) {
...
Status GcsFileSystem::RenameFile(const string& src, const string& target, std::unique_ptr<TransactionToken>* token) {
...
Status GcsFileSystem::IsDirectory(const string& fname, std::unique_ptr<TransactionToken>* token) {
  string bucket, object;
  TF_RETURN_IF_ERROR(ParseGcsPath(fname, true, &bucket, &object));
  if (object.empty()) {
    bool is_bucket;
...
void GcsFileSystem::FlushCaches(std::unique_ptr<TransactionToken>* token) {
  tf_shared_lock l(block_cache_lock_);
