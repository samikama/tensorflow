...
class RamFileSystem : public FileSystem {
 public:
  ...
  Status CreateDir(const string& dirname, std::unique_ptr<TransactionToken>* token=nullptr) override { return Status::OK(); }

  Status RecursivelyCreateDir(const string& dirname, std::unique_ptr<TransactionToken>* token=nullptr) override {
    return Status::OK();
  }

  Status DeleteDir(const string& dirname, std::unique_ptr<TransactionToken>* token=nullptr) override { return Status::OK(); }

  Status GetFileSize(const string& fname, uint64* file_size, std::unique_ptr<TransactionToken>* token=nullptr) override {
    mutex_lock m(mu_);
    if (fs_.find(fname) != fs_.end()) {
