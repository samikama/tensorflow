...
class TmpDirFileSystem : public NullFileSystem {
 public:
  Status FileExists(const string& dir, std::unique_ptr<TransactionToken>* token=nullptr) override {
  ...
  Status CreateDir(const string& dir, std::unique_ptr<TransactionToken>* token=nullptr) override {
    StringPiece scheme, host, path;
    io::ParseURI(dir, &scheme, &host, &path);
    if (scheme != "tmpdirfs") {
  ...
  Status IsDirectory(const string& dir, std::unique_ptr<TransactionToken>* token=nullptr) override {
    StringPiece scheme, host, path;
    io::ParseURI(dir, &scheme, &host, &path);
  ...
  void FlushCaches(std::unique_ptr<TransactionToken>* token=nullptr) override { flushed_ = true; }
