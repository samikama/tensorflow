...
class TmpDirFileSystem : public NullFileSystem {
 public:
  Status FileExists(const string& dir, TransactionToken* token=nullptr) override {
  ...
  Status CreateDir(const string& dir, TransactionToken* token=nullptr) override {
    StringPiece scheme, host, path;
    io::ParseURI(dir, &scheme, &host, &path);
    if (scheme != "tmpdirfs") {
  ...
  Status IsDirectory(const string& dir, TransactionToken* token=nullptr) override {
    StringPiece scheme, host, path;
    io::ParseURI(dir, &scheme, &host, &path);
  ...
  void FlushCaches(TransactionToken* token=nullptr) override { flushed_ = true; }
